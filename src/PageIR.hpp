...
#include "l3gdc20_lsm303dlhc_utils.h" // for setZeroAccelerometerGyroValue()
...
BDButton *const TouchButtonsIRControl[] = { &TouchButtonToggleSendReceive, &TouchButtonYaw_Plus, &TouchButtonYaw_Minus,
...
static BDSlider TouchSliderPitch1; // Vertical
...
void doIRButtons(BDButton *aTheTouchedButton, int16_t aValue) {
    if (aTheTouchedButton->mButtonHandle == TouchButtonsIRSend[0].mButtonHandle) {
        ...
        float tNumber = getNumberFromNumberPad(NUMBERPAD_DEFAULT_X, 0, COLOR16_GREEN);
    ...
    TEXT_SIZE_22, COLOR_PAGE_INFO, COLOR16_WHITE);
}
...
void doSetYawTrim(BDButton *aTheTouchedButton, int16_t aValue) {
    ...
    BUTTON_HEIGHT_4_LINE_3 + TEXT_SIZE_22_HEIGHT, sStringBuffer, TEXT_SIZE_11, COLOR16_RED, COLOR16_WHITE);
...
void startIRPage(void) {
    ...
    for (unsigned int i = 0; i < NUMBER_OF_IR_BUTTONS; ++i) {
        TouchButtonsIRSend[i].init(tXPos, 0, BUTTON_WIDTH_4, BUTTON_HEIGHT_5, COLOR16_GREEN, IRButtonStrings[i],
    ...
    COLOR16_RED, StringReceive, TEXT_SIZE_22, FLAG_BUTTON_DO_BEEP_ON_TOUCH, 0, &doToggleSendReceive);

    TouchButtonToggleLights.init(BUTTON_WIDTH_3_POS_2, BUTTON_HEIGHT_4_LINE_4, BUTTON_WIDTH_3, BUTTON_HEIGHT_4, 0, StringLights,
    TEXT_SIZE_22, FLAG_BUTTON_DO_BEEP_ON_TOUCH | FLAG_BUTTON_TYPE_TOGGLE_RED_GREEN, sLightValue, NULL);

    // Velocity
    TouchSliderVelocity.init(TEXT_SIZE_11_WIDTH, BUTTON_HEIGHT_5_LINE_2 + 12, SLIDER_DEFAULT_BAR_WIDTH * 2,
    VELOCITY_SLIDER_SIZE, VELOCITY_SLIDER_SIZE - 10, 0, COLOR16_BLUE, COLOR16_GREEN, FLAG_SLIDER_SHOW_BORDER, &doVelocitySlider);
    TouchSliderVelocity.setCaptionProperties(TEXT_SIZE_11, FLAG_SLIDER_VALUE_CAPTION_ALIGN_LEFT, SLIDER_DEFAULT_VALUE_MARGIN, COLOR16_RED,
    BACKGROUND_COLOR);
    TouchSliderVelocity.setCaption(StringVelocity);
    TouchSliderVelocity.setPrintValueProperties(TEXT_SIZE_11, FLAG_SLIDER_VALUE_CAPTION_ALIGN_LEFT, 4 + TEXT_SIZE_11,
    COLOR16_BLUE, BACKGROUND_COLOR);

    // PITCH - vertical
    TouchSliderPitch1.init(70, BUTTON_HEIGHT_5_LINE_3, SLIDER_DEFAULT_BAR_WIDTH, PITCH_MAX_VALUE,
    PITCH_ZERO_VALUE, PITCH_ZERO_VALUE, COLOR16_BLACK, COLOR16_GREEN, FLAG_SLIDER_VALUE_BY_CALLBACK, NULL);
    TouchSliderPitch1.setBarBackgroundColor(COLOR16_YELLOW);
    TouchSliderPitch1.setCaptionProperties(TEXT_SIZE_11, FLAG_SLIDER_VALUE_CAPTION_ALIGN_LEFT, SLIDER_DEFAULT_VALUE_MARGIN, COLOR16_RED,
    BACKGROUND_COLOR);
    TouchSliderPitch1.setCaption(StringPitch);
    TouchSliderPitch1.setPrintValueProperties(TEXT_SIZE_11, FLAG_SLIDER_VALUE_CAPTION_ALIGN_LEFT, 4 + TEXT_SIZE_11,
    COLOR16_BLUE, BACKGROUND_COLOR);

    // YAW - horizontal
    TouchSliderYaw.init(BUTTON_WIDTH_3_POS_2, BUTTON_HEIGHT_5_LINE_3, SLIDER_DEFAULT_BAR_WIDTH,
    YAW_MAX_VALUE - YAW_MIN_VALUE, YAW_ZERO_VALUE, YAW_ZERO_VALUE, COLOR16_BLACK, COLOR16_GREEN,
            FLAG_SLIDER_IS_HORIZONTAL | FLAG_SLIDER_VALUE_BY_CALLBACK, NULL);
    TouchSliderYaw.setBarBackgroundColor(COLOR16_YELLOW);
    TouchSliderYaw.setCaptionProperties(TEXT_SIZE_11, FLAG_SLIDER_VALUE_CAPTION_ALIGN_MIDDLE, SLIDER_DEFAULT_VALUE_MARGIN, COLOR16_RED,
    BACKGROUND_COLOR);
    TouchSliderYaw.setCaption(StringYaw);
    TouchSliderYaw.setPrintValueProperties(TEXT_SIZE_11, FLAG_SLIDER_VALUE_CAPTION_ALIGN_LEFT, SLIDER_DEFAULT_VALUE_MARGIN, COLOR16_BLUE,
    BACKGROUND_COLOR);

    TouchButtonYaw_Minus.init(BUTTON_WIDTH_3_POS_2,
    BUTTON_HEIGHT_4_LINE_3 + BUTTON_HEIGHT_4 - BUTTON_HEIGHT_6, BUTTON_WIDTH_10, BUTTON_HEIGHT_6, COLOR16_RED, "-",
    ...
    BUTTON_HEIGHT_4_LINE_3 + BUTTON_HEIGHT_4 - BUTTON_HEIGHT_6, BUTTON_WIDTH_8, BUTTON_HEIGHT_6, COLOR16_RED, "+",
    TEXT_SIZE_22, FLAG_BUTTON_DO_BEEP_ON_TOUCH, 3, &doSetYawTrim);
...
//        sIRSendData.command = (1 << 15) | (TouchButtonToggleLights.getValue() << 14) | (tPitchValue << 8); // set ModelSelect(1) + Light + 6*Pitch
        ...
        snprintf(sStringBuffer, sizeof sStringBuffer, "%2X %1X %2X %1X %1X %2X %1X %1X %s F=%d", sVelocitySendValue,
                sIRReceiveData.address >> 7 & 0x01, sYawValue, (sIRReceiveData.command >> 11) & 0x01, tLightValue, sPitchValue,
                tChecksum, tMyChecksum, irmp_protocol_names[sIRReceiveData.protocol], sIRReceiveData.flags);
        BlueDisplay1.drawText(2, BUTTON_HEIGHT_5 + 2, sStringBuffer, TEXT_SIZE_11, COLOR_PAGE_INFO, COLOR16_WHITE);
    ...
    if (sPitchValue != tPitchValue) {
        ...
        TouchSliderPitch1.setValueAndDrawBar(sPitchValue);
    }
    ...
    if (sSend) {
        delay(IHELICOPTER_FRAME_REPEAT_PAUSE_TIME_INT);
    }
