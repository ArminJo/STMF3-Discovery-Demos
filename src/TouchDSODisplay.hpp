 ...
 * TouchDSODisplay.hpp
 *
 * Contains the display related functions for DSO.
 *
...
#ifndef _TOUCH_DSO_DISPLAY_HPP
#define _TOUCH_DSO_DISPLAY_HPP
...
void computeAndDrawFFT(void) {
    ...
    ChartFFT.initXLabelInt(0, tFreqAtBin32, CHART_X_AXIS_SCALE_FACTOR_1, 4);
    ChartFFT.setXDataFactor(FFT_DISPLAY_SCALE_FACTOR_X);  // 2 for 320 display
...
void clearFFTValuesOnDisplay(void) {
    // Clear old fft area
    BlueDisplay1.fillRectRel(0, REMOTE_DISPLAY_HEIGHT - HORIZONTAL_GRID_HEIGHT, REMOTE_DISPLAY_WIDTH, HORIZONTAL_GRID_HEIGHT,
            COLOR_BACKGROUND_DSO);
}
...
void printInfo(bool aRecomputeValues) {
    if (DisplayControl.DisplayPage != DSO_PAGE_CHART || DisplayControl.showInfoMode == INFO_MODE_NO_INFO) {
        return;
    }
...
// number of digits to be printed after the decimal point
    ...
    if (DisplayControl.showInfoMode == INFO_MODE_LONG_INFO) {
        ...
        BlueDisplay1.drawText(0, FONT_SIZE_INFO_LONG_ASC + (2 * FONT_SIZE_INFO_LONG), sStringBuffer, FONT_SIZE_INFO_LONG,
...
// check for DataBufferDisplayStart
    if (DataBufferControl.DataBufferDisplayStart
            > DataBufferControl.DataBufferEndPointer
                    - (Chart::reduceIntWithIntegerScaleFactor(REMOTE_DISPLAY_WIDTH, DisplayControl.XScale) - 1)) {
...
int scrollChart(int aValue) {
    ...
    if (DisplayControl.DisplayPage == DSO_PAGE_CHART) {
        DataBufferControl.DataBufferDisplayStart += Chart::reduceIntWithIntegerScaleFactor(aValue, DisplayControl.XScale);
