...
#define RECS80EXT_START_BIT_PULSE_LEN_MIN       ((uint8_t)(F_INTERRUPTS * RECS80EXT_START_BIT_PULSE_TIME * MIN_TOLERANCE_20 + 0.5) - 1)
#define RECS80EXT_START_BIT_PULSE_LEN_MAX       ((uint8_t)(F_INTERRUPTS * RECS80EXT_START_BIT_PULSE_TIME * MAX_TOLERANCE_20 + 0.5) + 1)
#define RECS80EXT_START_BIT_PAUSE_LEN_MIN       ((uint8_t)(F_INTERRUPTS * RECS80EXT_START_BIT_PAUSE_TIME * MIN_TOLERANCE_10 + 0.5) - 1)
#define RECS80EXT_START_BIT_PAUSE_LEN_MAX       ((uint8_t)(F_INTERRUPTS * RECS80EXT_START_BIT_PAUSE_TIME * MAX_TOLERANCE_10 + 0.5) + 1)
#define RECS80EXT_PULSE_LEN_MIN                 ((uint8_t)(F_INTERRUPTS * RECS80EXT_PULSE_TIME * MIN_TOLERANCE_20 + 0.5) - 1)
#define RECS80EXT_PULSE_LEN_MAX                 ((uint8_t)(F_INTERRUPTS * RECS80EXT_PULSE_TIME * MAX_TOLERANCE_20 + 0.5) + 1)
...
#define IHELICOPTER_START_BIT_PAUSE_LEN_MAX        ((uint8_t)(F_INTERRUPTS * IHELICOPTER_START_BIT_PAUSE_TIME * MAX_TOLERANCE_00 + 0.5) + 1 - 2)  // 4
#define IHELICOPTER_1_PULSE_LEN_MIN                ((uint8_t)(F_INTERRUPTS * IHELICOPTER_1_PULSE_TIME * MIN_TOLERANCE_10 + 0.5) - 1) // 11
#define IHELICOPTER_1_PULSE_LEN_MAX                ((uint8_t)(F_INTERRUPTS * IHELICOPTER_1_PULSE_TIME * MAX_TOLERANCE_10 + 0.5) + 1) // 15
#define IHELICOPTER_1_PAUSE_LEN_MIN                ((uint8_t)(F_INTERRUPTS * IHELICOPTER_1_PAUSE_TIME * MIN_TOLERANCE_10 + 0.5) - 1) // 9
#define IHELICOPTER_1_PAUSE_LEN_MAX                ((uint8_t)(F_INTERRUPTS * IHELICOPTER_1_PAUSE_TIME * MAX_TOLERANCE_10 + 0.5) + 1) // 13
#define IHELICOPTER_0_PULSE_LEN_MIN                ((uint8_t)(F_INTERRUPTS * IHELICOPTER_0_PULSE_TIME * MIN_TOLERANCE_10 + 0.5) - 1) // 5
#define IHELICOPTER_0_PULSE_LEN_MAX                ((uint8_t)(F_INTERRUPTS * IHELICOPTER_0_PULSE_TIME * MAX_TOLERANCE_30 + 0.5) + 1) // 10
#define IHELICOPTER_0_PAUSE_LEN_MIN                ((uint8_t)(F_INTERRUPTS * IHELICOPTER_0_PAUSE_TIME * MIN_TOLERANCE_40 + 0.5) - 1) // 2
#define IHELICOPTER_0_PAUSE_LEN_MAX                ((uint8_t)(F_INTERRUPTS * IHELICOPTER_0_PAUSE_TIME * MAX_TOLERANCE_20 + 0.5) + 1) // 7
#define AUTO_FRAME_REPETITION_LEN               (uint16_t)(F_INTERRUPTS * AUTO_FRAME_REPETITION_TIME + 0.5)       // use uint16_t!
#ifdef ANALYZE_LOCAL
#include <stdio.h> /* for printf */
uint8_t sPulsesIndex = 0;
uint8_t sPulses[2 * IHELICOPTER_COMPLETE_DATA_LEN];
#endif
...
char * irmp_protocol_names[IRMP_N_PROTOCOLS + 1] = { "UNKNOWN", "SIRCS", "NEC", "SAMSUNG", "MATSUSH", "KASEIKYO", "RECS80", "RC5",
        "DENON", "RC6", "SAMSG32", "APPLE", "RECS80EX", "NUBERT", "BANG OLU", "GRUNDIG", "NOKIA", "SIEMENS", "FDC", "RCCAR", "JVC",
        "RC6A", "NIKON", "RUWIDO", "IR60", "KATHREIN", "NETBOX", "NEC16", "NEC42", "LEGO", "THOMSON", "BOSE", "A1TVBOX",
        "IHELICOPTER", };
#endif
...
static uint8_t start_bit_counter = IHELICOPTER_START_BITS_NUMBER;

static const PROGMEM IRMP_PARAMETER ihelicopter_param = { IRMP_IHELICOPTER_PROTOCOL,                 // protocol:        ir protocol
        IHELICOPTER_1_PULSE_LEN_MIN,                                    // pulse_1_len_min: minimum length of pulse with bit value 1
        IHELICOPTER_1_PULSE_LEN_MAX,                                    // pulse_1_len_max: maximum length of pulse with bit value 1
        IHELICOPTER_1_PAUSE_LEN_MIN,                                    // pause_1_len_min: minimum length of pause with bit value 1
        IHELICOPTER_1_PAUSE_LEN_MAX,                                    // pause_1_len_max: maximum length of pause with bit value 1
        IHELICOPTER_0_PULSE_LEN_MIN,                                    // pulse_0_len_min: minimum length of pulse with bit value 0
        IHELICOPTER_0_PULSE_LEN_MAX,                                    // pulse_0_len_max: maximum length of pulse with bit value 0
        IHELICOPTER_0_PAUSE_LEN_MIN,                                    // pause_0_len_min: minimum length of pause with bit value 0
        IHELICOPTER_0_PAUSE_LEN_MAX,                                    // pause_0_len_max: maximum length of pause with bit value 0
        IHELICOPTER_ADDRESS_OFFSET,                                            // address_offset:  address offset
        IHELICOPTER_ADDRESS_OFFSET + IHELICOPTER_ADDRESS_LEN,                  // address_end:     end of address
        IHELICOPTER_COMMAND_OFFSET,                                            // command_offset:  command offset
        IHELICOPTER_COMMAND_OFFSET + IHELICOPTER_COMMAND_LEN,                  // command_end:     end of command
        IHELICOPTER_COMPLETE_DATA_LEN,                                         // complete_len:    complete length of frame
        IHELICOPTER_STOP_BIT,                                                  // stop_bit:        flag: frame has stop bit
        IHELICOPTER_LSB,                                                       // lsb_first:       flag: LSB first
        IHELICOPTER_FLAGS                                                      // flags:           some flags
        };
...
void irmp_init(void) {
    ...
    /* GPIO Configuration */
#if defined (ARM_STM32L1XX) || defined (ARM_STM32F4XX) || defined  (ARM_STM32F30X)
    GPIO_InitStructure.Mode = GPIO_MODE_INPUT;
    GPIO_InitStructure.Pull = GPIO_NOPULL;
    GPIO_InitStructure.Speed = GPIO_SPEED_LOW;
    GPIO_InitStructure.Pin = IRMP_BIT_NUMBER;
...
static void irmp_store_bit(uint8_t value) {
#if IRMP_SUPPORT_GRUNDIG_NOKIA_IR60_PROTOCOL == 1
...
uint8_t irmp_ISR(void) {
    ...
    static uint8_t irmp_pulse_time = 0;                                        // count bit time for pulse
    static PAUSE_LEN irmp_pause_time = 0;                                        // count bit time for pause
...
#endif // IRMP_SUPPORT_RC6_PROTOCOL == 1
#if IRMP_SUPPORT_IHELICOPTER_PROTOCOL == 1
                    if (irmp_pulse_time >= IHELICOPTER_START_BIT_PULSE_LEN_MIN
                            && irmp_pulse_time <= IHELICOPTER_START_BIT_PULSE_LEN_MAX
                            && irmp_pause_time >= IHELICOPTER_START_BIT_PAUSE_LEN_MIN
                        ...
                        if (start_bit_counter != 0) {
                            ...
                            start_bit_counter = IHELICOPTER_START_BITS_NUMBER;
                            ANALYZE_PRINTF ("protocol = IHELICOPTER, start bit timings: pulse: %3d - %3d, pause: %3d - %3d\n",
                                    IHELICOPTER_START_BIT_PULSE_LEN_MIN, IHELICOPTER_START_BIT_PULSE_LEN_MAX,
                                    IHELICOPTER_START_BIT_PAUSE_LEN_MIN, IHELICOPTER_START_BIT_PAUSE_LEN_MAX);
                            irmp_param_p = (IRMP_PARAMETER *) &ihelicopter_param;
                        }
                    } else
#endif // IRMP_SUPPORT_IHELICOPTER_PROTOCOL == 1
                    {
...
//                              irmp_busy_flag = FALSE;
                                ...
                                irmp_command = irmp_tmp_command;
                                irmp_address = irmp_tmp_address;
...
//                                irmp_pause_time         = 0;
                ...
                if (got_light) {
                    ANALYZE_PRINTF("%8.3fms [bit %2d: pulse = %3d, pause = %3d] ", (double) (time_counter * 1000) / F_INTERRUPTS,
...
//                          irmp_busy_flag = FALSE;
                    ...
                    if (irmp_pulse_time >= irmp_param.pulse_1_len_min && irmp_pulse_time <= irmp_param.pulse_1_len_max
...
#ifdef ANALYZE_LOCAL
                        sPulses[sPulsesIndex++] = irmp_pulse_time;
                        sPulses[sPulsesIndex++] = irmp_pause_time;
#endif
                        ANALYZE_PRINTF("timing of 1 pulse %d pause %d", irmp_pulse_time, irmp_pause_time);ANALYZE_PUTCHAR ('1');ANALYZE_NEWLINE ();
                        irmp_store_bit(1);
                        wait_for_space = 0;
                        ...
                        sPulses[sPulsesIndex++] = irmp_pause_time;
#endif
                        ANALYZE_PRINTF("0  %d %d", irmp_pulse_time, irmp_pause_time);ANALYZE_PUTCHAR ('0');ANALYZE_NEWLINE ();
                        irmp_store_bit(0);
                        wait_for_space = 0;
                    } else
...
#ifdef ANALYZE_LOCAL
                int i;
                for (i = 0; i < 2 * IHELICOPTER_COMPLETE_DATA_LEN; i = i + 2) {
                    printf("%d-%d ", sPulses[i], sPulses[i + 1]);
                }
                printf(" ---\n");
                sPulsesIndex = 0;
#endif
