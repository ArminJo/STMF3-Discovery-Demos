...
TIM_HandleTypeDef TIM_IRSND_Handle;
#else
...
#  elif defined (ARM_STM32)                             // STM32
        //TODO
        HAL_TIM_OC_Start(&TIM_IRSND_Handle, TIM_CHANNEL_1);
...
#  elif defined (ARM_STM32)                             // STM32
 //       __HAL_TIM_DISABLE(&TIM_IRSND_Handle);
        //TODO
        HAL_TIM_OC_Stop(&TIM_IRSND_Handle, TIM_CHANNEL_1);
...
#    else
        TimeBaseFreq = HAL_RCC_GetPCLK2Freq();
#    endif
    }
    ...
    /* Set frequency */
    __HAL_TIM_SetAutoreload(&TIM_IRSND_Handle, freq - 1);
    // 50 % duty cycle
    __HAL_TIM_SetCompare(&TIM_IRSND_Handle, TIM_CHANNEL_1, (freq + 1) / 2);
#  else                                                                                     // AVR
...
#    elif defined (ARM_STM32F30X)
    // Initializing with CUBE library
    __GPIOC_CLK_ENABLE();
    GPIO_InitStructure.Mode = GPIO_MODE_AF_PP;
    GPIO_InitStructure.Pull = GPIO_NOPULL;
    GPIO_InitStructure.Speed = GPIO_SPEED_LOW;
    GPIO_InitStructure.Pin = IRSND_BIT_NUMBER;
    GPIO_InitStructure.Alternate = GPIO_AF1_TIM17;
...
#    else
    // Initializing with CUBE library
    CONCAT(CONCAT(__TIM, IRSND_TIMER_NUMBER), _CLK_ENABLE());
    //RCC_APB2PeriphClockCmd(IRSND_TIMER_RCC, ENABLE);
#    endif

    TIM_IRSND_Handle.Instance = IRSND_TIMER;
    /* Time base configuration */
    /* Time base configuration */
    TIM_IRSND_Handle.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
    TIM_IRSND_Handle.Init.Prescaler = 0x0; // no prescaler
    TIM_IRSND_Handle.Init.CounterMode = TIM_COUNTERMODE_UP;
    TIM_IRSND_Handle.Init.Period = -1;     // set dummy value (don't set to 0), will be initialized later
    HAL_TIM_Base_Init(&TIM_IRSND_Handle);

    /* PWM1 Mode configuration */
    TIM_OC_InitTypeDef TIM_OCInitStructure;
    TIM_OCInitStructure.OCMode = TIM_OCMODE_PWM1;
    TIM_OCInitStructure.OCFastMode = TIM_OCFAST_DISABLE;
    TIM_OCInitStructure.Pulse = 0; // will be initialized later
    TIM_OCInitStructure.OCPolarity = TIM_OCPOLARITY_HIGH;
    TIM_OCInitStructure.OCIdleState = TIM_OCIDLESTATE_RESET;
    TIM_OCInitStructure.OCNPolarity = TIM_OCNPOLARITY_HIGH;
    TIM_OCInitStructure.OCNIdleState = TIM_OCNIDLESTATE_RESET;
    HAL_TIM_PWM_ConfigChannel(&TIM_IRSND_Handle, &TIM_OCInitStructure, TIM_CHANNEL_1);

    /* Preload configuration */
    //    TIM_ARRPreloadConfig(IRSND_TIMER, ENABLE);
    TIM_IRSND_Handle.Instance->CR1 |= TIM_CR1_ARPE;
    __HAL_TIM_ENABLE(&TIM_IRSND_Handle);
