...
/*
 * Enabling both is a bit strange, because it disables the bluetooth connected display and calls only the local implemented GUI functions
 * The bluetooth communication is disabled by defining USART_isBluetoothPaired() to false.
 */
//#define AUTOREPEAT_BY_PERIODIC_CALL_OF_CHECK_ALL_BUTTONS // Autorepeat relies on periodic calls of checkAllButtons() by main loop
