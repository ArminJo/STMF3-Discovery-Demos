...
#if !defined(AUTOREPEAT_BY_PERIODIC_CALL_OF_CHECK_ALL_BUTTONS)
TouchButtonAutorepeat *TouchButtonAutorepeat::sLastAutorepeatButtonTouched = NULL;
#endif
...
void TouchButtonAutorepeat::autorepeatTouchHandler(TouchButtonAutorepeat *aTheTouchedButton, int16_t aButtonValue) {
#if defined(AUTOREPEAT_BY_PERIODIC_CALL_OF_CHECK_ALL_BUTTONS)
    /*
     * Autorepeat is implemented by periodic calls of checkAllButtons() by main loop
    ...
    if(isFirstTouch()){
        ...
        sPeriodicCallbackPeriodMillis = aTheTouchedButton->mMillisFirstDelay;
        sCount =aTheTouchedButton-> mFirstCount;

    } else if (millis() - sLastMillisOfLastCallOfPeriodicTouchCallback  > sPeriodicCallbackPeriodMillis) {
        /*
         * Period has expired
         */
        switch (sState) {
        case AUTOREPEAT_BUTTON_STATE_AFTER_FIRST_DELAY:
            ...
            sPeriodicCallbackPeriodMillis = aTheTouchedButton->mMillisFirstRate;
        ...
        case AUTOREPEAT_BUTTON_STATE_FIRST_PERIOD:
            if (sCount == 0) {
                ...
                sPeriodicCallbackPeriodMillis = aTheTouchedButton->mMillisSecondRate;
...
#  endif
     ...
     */
    // Touch down goes here => register delay callback
...
#endif // !defined(AUTOREPEAT_BY_PERIODIC_CALL_OF_CHECK_ALL_BUTTONS)
